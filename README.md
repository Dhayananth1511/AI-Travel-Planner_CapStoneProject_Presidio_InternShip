...
graph TD
    ...
    InputGoal --> Orchestrator["AI Planner Agent<br/>(Orchestrator)"]:::agent
    ...
    SplitTasks --> DestAgent["Destination Agent<br/>Choose Destination"]:::agent
    SplitTasks --> BudgetAgent["Budget Agent<br/>Estimate Total Cost"]:::agent
    SplitTasks --> ItinAgent["Itinerary Agent<br/>Generate Day-by-Day Plan"]:::agent
    
    DestAgent --> Coordinator["Coordinator Agent<br/>Combine Outputs"]:::agent
...
```text
       ...
       └───────────┬───────────┘                           └───────────┬───────────┘
                   ...
                   ▼                         ▼                         ▼
       ┌───────────────────────┐ ┌───────────────────────┐ ┌───────────────────────┐
       │   Destination Agent   │ │     Budget Agent      │ │    Itinerary Agent    │
       │  Choose Destination   │ │  Estimate Total Cost  │ │ Generate Day-by-Day   │
       └───────────┬───────────┘ └───────────┬───────────┘ └───────────┬───────────┘
                   │                         │                         │
                   └─────────────────────────┼─────────────────────────┘
                                             │
...
graph TD
    ...
    Goal(["User Goal"]):::startEnd --> Planner["Planner Agent"]:::agent
    ...
    CheckBudget -->|No| CheckItinerary{"Need Itinerary?"}:::process
    BudgetAgent --> CheckItinerary
    ...
    %% Wrap-up
    Coordinator --> GenPlan["Generate Final Plan"]:::process
...
graph TD
    ...
    subgraph ServerTier ["Backend Server (Express.js)"]
        ...
        subgraph agents ["AI Agent Cluster"]
            PlanAgent["Planner Agent"]:::backend
        ...
        PlanAgent --> Coord["Coordinator Agent"]:::backend
        BudAgent --> Coord
        ItinAgent --> Coord
    end
...
└─────┬─────┘                 └──────┬──────┘                └─────┬─────┘
      ...
      ▼                              ▼                             ▼
┌───────────┐                 ┌─────────────┐                ┌───────────┐
│  MongoDB  │◄────────────────┤ OpenRouter  │                │ Database  │
│ Database  │    Save Trip    │ LLM API     │                │   Check   │
└───────────┘                 └──────┬──────┘                └───────────┘
                                     │ (Tool Call)
                                     ▼
                              ┌─────────────┐
