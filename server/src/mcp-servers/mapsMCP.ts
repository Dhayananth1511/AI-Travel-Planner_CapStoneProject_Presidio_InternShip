...
export async function getPlacesNearby(
  ...
  if (apiKey && !apiKey.includes('REPLACE_WITH')) {
    try {
      ...
      // Fetch attractions
      const attrUrl = `https://api.geoapify.com/v2/places?categories=tourism.attraction,tourism.sights,natural.viewpoint,natural.valley,natural.peak,natural.forest,leisure.park,leisure.nature_reserve&filter=circle:${coords.lon},${coords.lat},15000&limit=40&apiKey=${apiKey}`;
